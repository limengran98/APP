...
def load_data(args, split=None, seed=None, verbose=False, normalize=False,
    ...
    if dataset in ["Roman-empire", "Amazon-ratings", "Minesweeper", "Tolokers"]:
        ...
        edge_index = fulldata['edge_index']      # Edge indices
        node_feat = fulldata['node_feat']        # Node features
        label = np.array(fulldata['label'], dtype=np.int32).flatten()  # Node labels
        # Convert to PyTorch tensors
        edge_index = torch.tensor(edge_index, dtype=torch.long)
        x = torch.tensor(node_feat, dtype=torch.float)   # Node features as float tensor
        y = torch.tensor(label, dtype=torch.long)        # Labels as long tensor
